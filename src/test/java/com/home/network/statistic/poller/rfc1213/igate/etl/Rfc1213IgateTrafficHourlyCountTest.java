...
class Rfc1213IgateTrafficHourlyCountTest {
    ...
    void testObtainFirstSqlQuery() {
        ...
        count.setInBytes(100);
        count.setOutBytes(200);

        // Act
        String query = count.obtainFirstSqlQuery();

        // Assert
        String expected = """
                select '2023-10-01' as `date`, '43200' as `time`, '12345' as `if_phys_address`, 'eth0' as `if_descr`, '300' as `transmission_bytes_val`
                """;
    ...
    void testObtainSqlQuery() {
        ...
        // Act
        String query = count.obtainSqlQuery();

        // Assert
        String expected = """
                union all
                select '2023-10-01', '43200', '12345', 'eth0', '300'
                """;
        assertEquals(expected.trim(), query.trim());
    }

    @Test
    void testObtainFirstSqlQueryStatic() {
        // Arrange
        ...
        count1.setInBytes(100);
        count1.setOutBytes(200);
        ...
        count2.setInBytes(50);
        count2.setOutBytes(75);

        Map<Rfc1213IgateTrafficHourlyCount, Rfc1213IgateTrafficHourlyCount> map = new LinkedHashMap<>();
        map.put(count1, count1);
        map.put(count2, count2);

        // Act
        String query = Rfc1213IgateTrafficHourlyCount.obtainFirstSqlQuery(map);
        ...
        String expected2 = """
                union all
                select '2023-10-01', '46800', '67890', 'eth1', '125'
                """.trim();
        assertTrue(query.contains(expected1));
        assertTrue(query.contains(expected2));
    }
